    ...
    public class PlayModeScript
    {
        ...
        public void Check_For_Main_Character_In_Scene_Test()
        {
            ...
            for (int i = 1; i < 8; i++)
            {
                ...
                string FailOutput = "Main Character not found in scene index " + i + "."; //an output for which scene doesn't have a "MainCharacter" game object
                Assert.IsNotNull(MC, FailOutput); //Check to see that variable MC "is not null" (or holds *something* where that something should be the MainCharacter).
