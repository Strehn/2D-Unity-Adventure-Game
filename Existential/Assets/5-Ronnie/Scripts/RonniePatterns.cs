...
public class RonnieIterate : MonoBehaviour {
    ...
    static public int first() { 
        return index; //initialize first variable
    } 
    static public int next() { 
        return index++; //iterate to next 
    } 
    static public bool isDone(int max) { 
        return index == max; //return if iteration is done
    } 
}